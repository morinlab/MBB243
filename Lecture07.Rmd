...
```{r read,echo=T}
library(readr)
exp = read_csv("data/GSE125966_GOYA_mini.csv")
...
```{r,eval=T,echo=F, warning=F,message=F}
filter(exp, sample1 > -50) %>% ggplot(aes(x=log10(sample1))) + geom_histogram()
...
exp = exp %>% rename("Gene_ID"="X1")
# use pivot_longer function to bring sample1 and sample2 columns into one column

exp_tidy = pivot_longer(exp,-Gene_ID,names_to="sample",values_to="expression")
```
